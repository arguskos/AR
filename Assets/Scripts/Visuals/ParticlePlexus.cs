            ...
            for (int i = 0; i < particleCount; i++)
            {
                ...
                for (int j = i + 1; j < particleCount; j++)
                {
                    ...
                    if (distanceSqr <= maxDistanceSqr)
                    {
                        ...
                        //Take most faded particle color
                        Color finalcolor;
                        if (p1_color.a <= p2_color.a)
                        {
                            finalcolor = p1_color;
                        }
                        else
                        ...
                        float actualdistance = 1.0f - (Mathf.Abs((p1_position - p2_position).magnitude)/maxDistance) * (Mathf.Abs((p1_position - p2_position).magnitude) / maxDistance);
                        actualdistance = (actualdistance < 0) ? 0 : actualdistance;
                        finalcolor.a *= actualdistance;

                        lr.SetPosition(0, p1_position);
                        lr.SetPosition(1, p2_position);
                        lr.material.color = finalcolor;
